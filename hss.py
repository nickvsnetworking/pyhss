...
class DiameterRequestHandler(socketserver.BaseRequestHandler):
    ...
    firstloop = 0
    def setup(self):
        logging.info('New connection from ' + str(self.client_address[0]))
        while 1:
            data = self.request.recv(32)
            if not data:
                logging.info("Connection closed by " + str(self.client_address[0]))
                break
            try:            
            ...
            #SLh LCS-Routing-Info-Answer
                ...
                for keys in packet_vars:
                    logging.debug(keys)
                    logging.debug("\t" + str(packet_vars[keys]))
                logging.debug(avps)
                logging.info("Sending negative response")
                packet_vars, avps
                response = diameter.Respond_ResultCode(packet_vars, avps, 3001)      #Generate 	DIAMETER_COMMAND_UNSUPPORTED response
