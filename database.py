...
class MongoDB:
    ...
    def __init__(self):
        logging.info("Configured to use MongoDB server: " +
                     str(yaml_config['database']['mongodb']['mongodb_server']))
        self.server = {}
    ...
    def QueryDB(self, imsi):
        ...
        myclient = self.pymongo.MongoClient(
            "mongodb://" + str(self.server['mongodb_server']) + ":" + str(self.server['mongodb_port']) + "/")
    ...
    def GetSubscriberInfo(self, imsi):
        ...
        try:
            ...
            raise ValueError(
                "Failed to pull subscriber details for IMSI " + str(imsi) + " from MongoDB")
        ...
        for x in mydoc:
            ...
            try:
                ...
                logging.debug(
                    "Subscriber " + str() + " has not attached before - Generating new SQN and RAND")
            ...
            subscriber_details['APN_list'] = apn_list[:-1]
        ...
        raise ValueError(
            "Mongodb has no matching subscriber details for IMSI " + str(imsi) + " from MongoDB")
    ...
    def UpdateSubscriber(self, imsi, sqn, rand):
        ...
        try:
            logging.debug("Updating SQN for imsi " +
                          str(imsi) + " to " + str(sqn))
            # Search for user in MongoDB database
            myclient = self.pymongo.MongoClient(
                "mongodb://" + str(self.server['mongodb_server']) + ":" + str(self.server['mongodb_port']) + "/")
            ...
            raise ValueError(
                "Failed update SQN for subscriber " + str(imsi) + " in MongoDB")
...
class MSSQL:
    ...
    def __init__(self):
        import pymssql
        logging.info("Configured to use MS-SQL server: " +
                     str(yaml_config['database']['mssql']['server']))
        self.server_conf = yaml_config['database']['mssql']

        try:
            self.conn = pymssql.connect(
                self.server_conf['server'], self.server_conf['username'], self.server_conf['password'], self.server_conf['database'])
            self.cursor = self.conn.cursor(as_dict=True)
            logging.info("Connected to MSSQL Server")
        except:
            # If failed to connect to server
            logging.fatal("Failed to connect to MSSQL server at " +
                          str(self.server['server']))
            raise OSError("Failed to connect to MSSQL server at " +
                          str(self.server['server']))
            sys.exit()
    ...
    def GetSubscriberInfo(self, imsi):
        try:
            logging.debug(
                "Getting subscriber info from MSSQL for IMSI " + str(imsi))
            subscriber_details = {}
            # try:
            ...
            if str(result['known_imsi']) != '1':
                raise ValueError("MSSQL reports IMSI " +
                                 str(imsi) + " not attached with SIM")

            # subscriber_status: -1 –Blocked or 0-Active
            if str(result['subscriber_status']) != '0':
                raise ValueError(
                    "MSSQL reports Subscriber Blocked for IMSI " + str(imsi))

            apn_id = result['apn_configuration']

            logging.debug(
                "Running hss_get_subscriber_data_v2 for imsi " + str(imsi))
            ...
            # Format MSISDN
            subscriber_details['msisdn'] = str(
                result['region_subscriber_zone_code']) + str(result['msisdn'])
            subscriber_details['msisdn'] = subscriber_details['msisdn'].split(
                ';')[-1]
            ...
            logging.debug("Final subscriber data for IMSI " +
                          str(imsi) + " is: " + str(subscriber_details))
            return subscriber_details
        except:
            raise ValueError(
                "MSSQL failed to return valid data for IMSI " + str(imsi))
    ...
    def GetSubscriberLocation(self, *args, **kwargs):
        ...
        if 'imsi' in kwargs:
            ...
            try:
                ...
                logging.debug(
                    "Calling hss_get_mme_identity_by_info with IMSI " + str(imsi))
                self.conn.execute_query(
                    'hss_get_mme_identity_by_info ' + str(imsi) + ';')
                logging.debug(self.conn)
            except:
                raise ValueError(
                    "MSSQL failed to run SP hss_get_mme_identity_by_info for IMSI " + str(imsi))
        elif 'msisdn' in kwargs:
            logging.debug("MSISDN present - Searching based on MSISDN")
            try:
                msisdn = kwargs.get('msisdn', None)
                logging.debug(
                    "Calling hss_get_mme_identity_by_info with msisdn " + str(msisdn))
                self.conn.execute_query(
                    'hss_get_mme_identity_by_info ' + str(msisdn) + ';')
                logging.debug(self.conn)
            except:
                raise ValueError(
                    "MSSQL failed to run SP hss_get_mme_identity_by_info for msisdn " + str(msisdn))
        else:
    ...
    def UpdateSubscriber(self, imsi, sqn, rand, *args, **kwargs):
        try:
            ...
            try:
                ...
                sql = 'hss_auth_get_ki_v2 @imsi=' + \
                    str(imsi) + ', @NBofSeq=' + str(sqn) + ';'
                logging.debug(sql)
                self.conn.execute_query(sql)
                logging.debug(self.conn)
            except:
                logging.error("MSSQL failed to run SP hss_auth_get_ki_v2 with SQN " +
                              str(sqn) + " for IMSI " + str(imsi))
                raise ValueError(
                    "MSSQL failed to run SP hss_auth_get_ki_v2 with SQN " + str(sqn) + " for IMSI " + str(imsi))

            # If optional origin_host kwag present, store UE location (Serving MME) in Database
            if 'origin_host' in kwargs:
                logging.debug(
                    "origin_host present - Updating MME Identity of subscriber")
                ...
                if len(origin_host) != 0:
                    try:
                        logging.debug(
                            "Updating MME Identity using SP hss_update_mme_identity")
                        logging.debug("Writing serving MME to database")
                        sql = 'hss_update_mme_identity @imsi=' + str(imsi) + ', @orgin_host=\'' + str(
                            origin_host) + '\', @Cancellation_Type=0, @ue_purged_mme=0;'
                        logging.debug(sql)
                        self.conn.execute_query(sql)
                    except:
                        logging.error("MSSQL failed to run SP hss_update_mme_identity with IMSI " + str(
                            imsi) + " and Origin_Host " + str(origin_host))
                else:
                    try:
                        logging.debug(
                            "Removing MME Identity as new MME Identity is empty")
                        sql = 'hss_delete_mme_identity @imsi=' + str(imsi)
                        logging.debug(sql)
                        self.conn.execute_query(sql)
                    except:
                        logging.error(
                            "MSSQL failed to run SP hss_delete_mme_identity with IMSI " + str(imsi))
            else:
                logging.debug(
                    "origin_host not present - not updating UE location in database")
        except:
            raise ValueError(
                "MSSQL failed to update SQN for IMSI " + str(imsi))

    def TestFail(self):
        logging.debug("Called testing route")
        sql = "exec hss_get_subscriber_data @IMSI = '204047905104651';"
        logging.debug(sql)
        try:
            ...
            logging.critical(
                "We have an OperationalError - This is usually due to dead DB connection - Shutting down")
            sys.exit()
...
class MySQL:
    ...
    def __init__(self):
        logging.info("Configured to use MySQL server: " +
                     str(yaml_config['database']['mysql']['server']))
    ...
    def GetSubscriberInfo(self, imsi):
        logging.debug(
            "Getting subscriber info from MySQL for IMSI " + str(imsi))
        self.cursor.execute(
            "select * from subscribers left join subscriber_apns on subscribers.imsi = subscriber_apns.imsi left join apns on subscriber_apns.apn_id = apns.apn_id where subscribers.imsi = " + str(imsi))
        subscriber_details = self.cursor.fetchall()
        return subscriber_details

    def UpdateSubscriber(self, imsi, sqn, rand, *args, **kwargs):
        logging.debug("Updating SQN for imsi " + str(imsi) + " to " + str(sqn))
        query = 'update subscribers set sqn = ' + \
            str(sqn) + ' where imsi = ' + str(imsi)
        logging.debug(query)
...
if __name__ == "__main__":
    try:
        ...
        print("Exception occoured - value " + str(ex))
        print("reinit DB")
        DB = MySQL()
    ...
    try:
        ...
        print("reinit DB")
        DB = MySQL()
    print("Made it back to the loop, sleeping")
    import time
    time.sleep(5)
    print("And running again...")
    try:
        print(DB.TestFail())
    except Exception as ex:
        print("Exception occoured - value " + str(ex))
