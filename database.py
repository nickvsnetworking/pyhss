...
def Update_Serving_APN(imsi, apn, pcrf_session_id, serving_pgw, ue_ip, propagate=True):
    DBLogger.debug("Called Update_Serving_APN() for sub " + str(imsi) + " and APN " + str(apn))
    ...
    for apn_id in apn_list:
        ...
        if str(apn_data['apn']).lower() == str(apn).lower():
            ...
            #Check if already a serving APN on record
                ...
                ServingAPN_resp = CreateObj(SERVING_APN, json_data)
                DBLogger.info("Created new Serving APN Entry: " + str(ServingAPN_resp))
