...
class DiameterAsync:
    ...
    async def getAvpDataAsync(self, avps, avp_code):
    ...
    async def decodeDiameterPacketAsync(self, data):
        packet_vars = {}
        avps = []
        ...
        avp_vars, remaining_avps = await(self.decodeAvpPacketAsync(avp_sum))
        avps.append(avp_vars)
        
        while len(remaining_avps) > 0:
            avp_vars, remaining_avps = await(self.decodeAvpPacketAsync(remaining_avps))
            avps.append(avp_vars)
        else:
            pass
        return packet_vars, avps

    async def decodeAvpPacketAsync(self, data):  
        ...
        if avp_vars['avp_length'] % 4  == 0:
            ...
            rounded_value = await(self.myRoundAsync(avp_vars['avp_length']))
        ...
        try:
            sub_avp_vars, sub_remaining_avps = await(self.decodeAvpPacketAsync(avp_vars['misc_data']))
            ...
            if int(sub_avp_vars['avp_code']) > 9999:
                ...
                while len(sub_remaining_avps) > 0:
                    sub_avp_vars, sub_remaining_avps = await(self.decodeAvpPacketAsync(sub_remaining_avps))
                    avp_vars['misc_data'].append(sub_avp_vars)
    ...
    async def getDiameterMessageTypeAsync(self, binaryData: str) -> dict:
        packet_vars, avps = await(self.decodeDiameterPacketAsync(binaryData))
        response = {}
    ...
    async def generateDiameterResponseAsync(self, binaryData: str) -> str:
        packet_vars, avps = await(self.decodeDiameterPacketAsync(binaryData))
        response = ''
