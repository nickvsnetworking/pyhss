...
class APN_Tests(unittest.TestCase):
    ...
    def test_B_create_APN(self):
        ...
        r = requests.put('http://localhost:5001/PyHSS/apn', data=json.dumps(self.__class__.template_data), headers=headers)
        self.__class__.apn_id = r.json()['apn_id']
        log.debug("Created APN ID " + str(self.__class__.apn_id))
        self.assertEqual(r.status_code, 200, "Status Code should be 200 OK")

    def test_C_Get_APN(self):
        r = requests.get('http://localhost:5001/PyHSS/apn/' + str(self.__class__.apn_id))
    ...
    def test_D_Patch_APN(self):
        ...
        r = requests.patch('http://localhost:5001/PyHSS/apn/' + str(self.__class__.apn_id), data=json.dumps(patch_template_data), headers=headers)
    ...
    def test_F_Delete_Patched_APN(self):
        r = requests.delete('http://localhost:5001/PyHSS/apn/' + str(self.__class__.apn_id))
        xres = {"Result": "OK"}
...
class AUC_Tests(unittest.TestCase):
    ...
    def test_B_create_AUC(self):
        headers = {"Content-Type": "application/json"}
        r = requests.put('http://localhost:5001/PyHSS/auc', data=json.dumps(self.__class__.template_data), headers=headers)
        self.__class__.auc_id = r.json()['auc_id']
        log.debug("Created AUC ID " + str(self.__class__.auc_id))
        self.assertEqual(r.status_code, 200, "Status Code should be 200 OK")

    def test_C_Get_AUC(self):
        r = requests.get('http://localhost:5001/PyHSS/auc/' + str(self.__class__.auc_id))
    ...
    def test_D_Patch_AUC(self):
        ...
        r = requests.patch('http://localhost:5001/PyHSS/auc/' + str(self.__class__.auc_id), data=json.dumps(patch_template_data), headers=headers)
        self.assertEqual(patch_template_data, r.json(), "JSON body should match input")
    ...
    def test_F_Delete_Patched_AUC(self):
        r = requests.delete('http://localhost:5001/PyHSS/auc/' + str(self.__class__.auc_id))
        xres = {"Result": "OK"}
        self.assertEqual(xres, r.json(), "JSON body should match " + str(xres))
